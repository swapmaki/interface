...
const NETWORK_URL = 'https://moonbeam-api.bwarelabs.com/56b40bad-4434-4137-8f4c-3ce847b7cd52'
...
export async function farms() {
  if (!farmsResult) {
    ...
    const ret = []
    const poolStaticInfo = POOLS[ChainId.MOONBEAM]
    ...
    for (const pool of poolsInfo) {
      ...
      if (!staticInfo.token1) {
        ret.push({
          ...
          baseSymbol: staticInfo.token0.symbol.toLowerCase().replace('wglmr', 'glmr'),
        ...
        ret.push({
          ...
          baseSymbol:
            token0info.symbol == 'SOLAR' || token1info.symbol == 'SOLAR'
              ? 'solar'
              : token0info.symbol == 'GLMR' || token1info.symbol == 'GLMR'
              ? 'movr'
